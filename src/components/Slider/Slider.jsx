    ...
    <div className='relative pb-4'>
        <div>
            {sliderData.map((item, index)=>{
                return (
                    <div key={item.id} 
                        className={parseInt(item.id) === slideIndex ? 'opacity-100 duration-700 ease-in-out scale-100' 
                            : "opacity-0 duration-700 ease-in-out scale-95"}>
                        <div>
                            {parseInt(item.id) === slideIndex && (
                            <img className='h-[850px] w-full'
        ...
        <div className="flex absolute bottom-14 left-[50%] translate-x-[-50%]">
        ...
        <div>
            <button 
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" 
                    viewBox="0 0 24 24" strokeWidth="2" stroke="#000" 
                    className="w-6 h-6">
